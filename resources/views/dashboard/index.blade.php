        ...
        <section class="our-webcoderskull padding-lg">
            <div class="container">
                <div class="alert alert-success" role="alert">
                    Top Emplyee Ranking: {{ \Carbon\Carbon::now()->format('F') }} -
                    ...
                    @foreach ($data['ranking'] as $item)
                        <li class="col-12 col-md-3 col-lg-3 @if ($loop->first) block @endif">
                            <div class="cnt-block equal-hight" style="height: 349px;">
                                <figure>
                                    @if (isset($item->image))
                                        <img src="{{ url(Storage::url('users/' . $item->id . '/' . $item->image)) }}"
                                            class="img-responsive" alt="{{ $item->name }}">
                                    @else
                                        <img src="{{ asset('img/default_avatar.jpg') }}" class="img-responsive"
                                            alt="{{ $item->name }}">
                                    @endif
                                </figure>
                                ...
                                <div class="rank">
                                    <div class="bg_rank">
                                        <center>
                                            <h3 class="rankNumber" title="Ranking Positon">Rank -{{ $rank }}
                                            </h3>
                                        </center>
                                    </div>
                                </div>
                ...
                <div class="col-6 col-lg-3">
                    <div class="app-card app-card-stat shadow-sm h-100">
                        <div class="app-card-body p-3 p-lg-4">
                            ...
                            <div class="stats-figure">৳{{ number_format($data['targetedCollection']->requiredAmount, 2) }}
                            </div>
                            <p>Required : {{ round($data['required'], 2) }} Tk <br>
                                @if ($data['average'] > $data['required'])
                                    <span class="text-success">Average : {{ round($data['average'], 2) }} Tk</span>
                                @else
                                    <span class="text-danger">Average: {{ round($data['average'], 2) }} Tk</span>
                                @endif
                            </p>
                        </div>
                        <a class="app-card-link-mask" href="#"></a>
                ...
                <div class="app-card app-card-stat shadow-sm h-100 bg-danger">
                    <div class="app-card-body p-3 p-lg-4  ">
                        ...
                        <div class="stats-figure text-white">
                            ৳{{ number_format($data['amountTotal'] - $data['duePayment'], 2) }}</div>
                    </div>
                    <a class="app-card-link-mask" href="#"></a>
                </div>
            </div>
                                ...
                                <tbody>
                                    @foreach ($data['reminderAdmin'] as $reminder)
                                        <tr>
                                            ...
                                            <td><a
                                                    href="{{ route('student_search', 'registration_number=' . $reminder->id) }}">{{ $reminder->name }}</a>
                                            </td>
                            ...
                            <div class="app-card app-card-chart h-10 shadow-sm">
                                ...
                                <div class="app-card-body p-4">
                                    <div class="chart-container">
                                        <canvas id="chart-all-student"></canvas>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <tbody>
                                @foreach ($data['reminderCro'] as $reminder)
                                    <tr>
                                        <th scope="row">{{ $reminder->id }}</th>
                                        <td><a
                                                href="{{ route('student_search_cro', 'registration_number=' . $reminder->id) }}">{{ $reminder->name }}</a>
                                        </td>
                                        <td>{{ $reminder->mobile }}</td>
                                        <td>{{ $reminder->note }}</td>
                                    </tr>
