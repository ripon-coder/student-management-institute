...
class BackupController extends Controller
{
    ...
    public function index(){
        ...
        $files = $disk->files('/CandleIt/');
        $backups = [];
        foreach ($files as $k => $f) {
           if (substr($f, -4) == '.zip' && $disk->exists($f)) {
               $backups[] = [
               'file_path' => $f,
               'file_name' => str_replace(config('laravel-backup.backup.name') . 'CandleIt/', '', $f),
  ...
  public function download($file_name) {
    $file = config('laravel-backup.backup.name') .'/CandleIt/'. $file_name;
...
public function delete($file_name){
    $disk = Storage::disk(config('laravel-backup.backup.destination.disks'));
    if ($disk->exists(config('laravel-backup.backup.name') . '/CandleIt/' . $file_name)) {
         $disk->delete(config('laravel-backup.backup.name') . '/CandleIt/' . $file_name);
