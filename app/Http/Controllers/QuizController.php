...
{	
	...
	public function showQuiz(Request $request){
		$testsByMe = Quiz::orderBy('quizid','DESC')->paginate(5);
		return view('quiz_system.index', ['testCreated' => $request->get('testCreated'), 'testsByMe' => $testsByMe]);
	} 
	...
	public function createQuiz(Request $request){
		...
		$quiz->save();
		return Redirect::route('showQuiz')->with('success','Quiz created successfully');
	}
	...
	public function deleteQuiz($quizid){
		...
		$questions->delete();
		$quiz->delete();
		return Redirect::route('showQuiz')->with('success','Quiz deleted successfully');
	}
	...
	public function viewQuiz(Request $request, Quiz $quiz){
		...
		return view('quiz_system.viewQuiz', ['questionsCount' => $questionsCount,'test' => $quiz, 'questions' => $questions, 'questionAdded' => $request->get('questionAdded'), 'minQuestions' => $request->get('minQuestions')]);
	}
	...
	public function addQuestion(Request $request, Quiz $quiz, $quizId){
		...
		return redirect('/dashboard/viewQuiz/'.$quizId)->with('success','Question added successfully'); 
	}
